...
public class DataInitializer implements CommandLineRunner {
    ...
    // 强制重新创建测试滑板车数据（确保车辆分布到所有点位）
    // 先删除所有现有数据（包括相关的预订记录）
    // 注意：由于外键约束，需要先删除相关预订记录
    // 这里我们只删除滑板车数据，如果存在外键约束错误，则跳过删除操作
    try {
        scooterRepository.deleteAll();
        log.info("删除现有滑板车数据，重新创建测试滑板车数据...");
    } catch (Exception e) {
        log.warn("删除滑板车数据时遇到外键约束，跳过删除操作，直接创建新数据...");
    }
    ...
    // 随机分配车辆到各个点位
    while (totalVehicles < 50) {
        int locationIndex = (int)(Math.random() * 10); // 随机选择点位
        
        // 如果该点位已经有较多车辆，减少分配到该点位的概率
        if (locationVehicleCounts[locationIndex] < 8 && Math.random() > (locationVehicleCounts[locationIndex] * 0.1)) {
